...
- Caching: Redis
- Payments: Stripe
- Social Auth: Laravel Socialite — Google (`laravel/socialite`) and Microsoft/Outlook (`socialiteproviders/microsoft`), expandable to other providers later
- Local development: Laravel Sail (Docker) with PostgreSQL, Redis, pgAdmin, and Mailpit

## Development Workflow
- All commands use the `sail` prefix: `sail artisan`, `sail composer`, `sail npm`
- `sail up` and `sail npm run dev` are always running during development
- **Edit existing migrations directly** — do not create new migrations during active development. Run `sail artisan migrate:fresh` to reset and replay.
- Tests run against the PostgreSQL `testing` database (auto-created by Sail)

## Build Order

We are building iteratively — POC → MVP → polish, adding features and fixes in each iteration. The order below reflects the planned sequence for initial development.

1. **Auth & Identity** — landing page, registration (email + social), login, 2FA, email verification, password reset, user model (ULID, username, tier, profile images)
2. **First Server Space** — Limbo server, invite codes, server membership, dashboard
3. **Public Profile** — `/@username` page, follow system
4. **Server Space v2** — page builder, block editor, visibility rules, demo access
5. **Payments** — Stripe integration, Loyalist subscription, server creation purchase
6. **Social Graph** — friends system, follower-gated content
7. **Productivity Tools** — Expense Planner, Life Planner (details TBD when phase begins)

## Development Methodology
- Follow a iterative, MVP-first approach — build the simplest possible version of each feature, then iterate based on user feedback and testing.
- Prioritize core features that enable the main value proposition (community + page builder) before building out secondary features (productivity tools, enhanced profile features, etc.).
- Use feature flags to manage the rollout of new features and allow for testing in production without affecting all users.
- Write comprehensive tests for all features, including unit tests, integration tests, and end-to-end tests to ensure reliability and a smooth user experience.
- Maintain clear and consistent documentation in the `docs/` directory to ensure that development is aligned on the product vision and implementation details.
...
- `blueprint/schema.md` — planned database schema, organised by build phase.
- `blueprint/profile.md` — public profile design and visibility rules.
- `blueprint/flow.md` — auth and access flows.
